...
def Build_app():
    ...
    english_app_language = ["Search for Pokémon to get information!",
                            "Insert Pokémon name here", "Search", "Clear", "Deletes the last\nsearch output."]
    german_app_language = ["Suche nach Pokémon um Informationen zu bekommen!",
                           "Pokémon Namen hier einfügen", "Suchen", "Löschen", "Löscht das letzte\nSuchergebnis."]
    ...
    try:
        ...
        with window(pokedex_version, width=520, height=900):
            ...
            Build_menu()
            # image logo
            # create some space for the image
            add_drawing("logo", width=520, height=250)
            add_separator()
            add_spacing(count=5)
            ...
            # Optional User Input
            add_input_text(
                "Input", width=415, hint=language[1], default_value='', on_enter=True, callback=Start_pokemon_check, label="")
            # Button
            add_spacing(count=5)
            add_button(language[2], callback=Start_pokemon_check)
            add_same_line()
            add_button(
                language[3], callback=Clear_last_result, tip=language[4])
            add_spacing(count=5)
        # place the image inside the space "logo"
        draw_image("logo", r"PokeDex.png", [115, 0], [
                   365, 250], tag="Pokemon")  # padding 25
...
def Build_menu():
    english_menu_language = ["Home", "More", "Go to project page", "See the logs", "Beta functions",
                             "Not working yet", "Credits", "Uranium", "Reset", "Help", "Settings", "Language", "German", "English"]
    german_menu_language = ["Home", "Mehr", "Projekt Seite", "Logs anzeigen", "Beta Funktionen",
                            "Noch nicht fertig", "Credits", "Uranium", "Reset", "Hilfe", "Einstellungen", "Sprache", "Deutsch", "Englisch"]
    if current_languge == "english":
    ...
    with menu_bar("Menu Bar", parent=pokedex_version):
        ...
        with menu(language[1], parent="Menu Bar"):
            ...
            add_menu_item(
                language[3], callback=show_logger, parent=language[1])
            with menu(language[4], label=language[5], parent=language[1]):
                add_menu_item(
                    language[6], callback=Build_credits, parent=language[4])
        with menu(language[10], parent="Menu Bar"):
            ...
            with menu(language[11], parent=language[10]):
                add_menu_item(
                    language[12], callback=Change_language_to_german, parent=language[11])
                add_menu_item(
                    language[13], callback=Change_language_to_english, parent=language[11])
...
def Build_help():
    ...
    add_text("[Version] " + pokedex_version,
             parent="help window", color=[200, 100, 100])
...
def Build_uranium():
    ...
    try:
        ...
        add_spacing(count=2)
        # Optional User Input
        add_input_text("Input", width=415, hint=language[1], default_value='',
                       on_enter=True, callback=Start_pokemon_uranium_check, label="")
        # Button
        add_spacing(count=5)
        add_button(language[2], callback=Start_pokemon_uranium_check)
        add_same_line()
        add_button(
            language[3], callback=Clear_last_uranium_result, tip=language[4])
        add_spacing(count=5)
    # place the image inside the space "logo"
    draw_image("logo", r"Uranium.png", [115, 0], [
               365, 250], tag="Pokemon")  # padding 25
...
def Start_pokemon_check():
    ...
    pokemon_name, pokemon_type, pokemon_type2, pokemon_evolution = Search(
        input_value)
    ...
    if len(pokemon_evolution) == 1:
        ...
        add_tab_button(Translator(
            pokemon_evolution[0]), parent="pokemon_tab_bar", callback=Change_input_value1)
    if len(pokemon_evolution) == 2:
        ...
        add_tab_button(Translator(
            pokemon_evolution[0]), parent="pokemon_tab_bar", callback=Change_input_value1)
        add_tab_button(Translator(
            pokemon_evolution[1]), parent="pokemon_tab_bar", callback=Change_input_value2)
    if len(pokemon_evolution) == 3:
        ...
        add_tab_button(Translator(
            pokemon_evolution[1]), parent="pokemon_tab_bar", callback=Change_input_value2)
        add_tab_button(Translator(
            pokemon_evolution[2]), parent="pokemon_tab_bar", callback=Change_input_value3)
...
def Start_pokemon_uranium_check():
    ...
    if search_runs == []:
        ...
        Delete_last_result()
    # get result from search function in pokedexx and set them to a single variable
    resulted_pokemon, resulted_pokemon2, resulted_pokemon3 = Uranium_Search(
        input_value)
    if resulted_pokemon != None:
        add_tab_bar("pokemon_tab_bar", parent="uranium")
        new_value1 = resulted_pokemon.get_name()
        add_tab_button(resulted_pokemon.get_name(
        ), parent="pokemon_tab_bar", callback=Change_uranium_input_value1)
    ...
    if resulted_pokemon2 != None:
        ...
        add_tab_button(resulted_pokemon2.get_name(
        ), parent="pokemon_tab_bar", callback=Change_uranium_input_value2)
        # add_tab("test3", parent="pokemon_tab_bar")
    if resulted_pokemon3 != None:
        new_value3 = resulted_pokemon3.get_name()
        add_tab_button(resulted_pokemon3.get_name(
        ), parent="pokemon_tab_bar", callback=Change_uranium_input_value3)
        # search_result += ", " + resulted_pokemon3.get_name()
    error_text = f"Pokémon '{input_value}' konnte nicht gefunden werden!"
    if resulted_pokemon == None:
        clear_drawing("logo")
        draw_image("logo", r"error_404.png", [
                   115, 0], [365, 250])  # padding 25
        ...
        urllib.request.urlretrieve(
            resulted_pokemon.get_picture(), "sprite.png")
        clear_drawing("logo")
        draw_image("logo", r"sprite.png", [115, 0], [365, 250])  # padding 25
