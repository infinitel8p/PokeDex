...
class Uranium(customtkinter.CTkFrame):
    def __init__(self, parent):
        super().__init__(master=parent)
        self.configure(fg_color="transparent")
        customtkinter.set_default_color_theme(
            os.path.join("themes", "green.json"))

        # Content
        # Image
        image = customtkinter.CTkImage(
            Image.open("images/Uranium.png"), size=(250, 250))
        ...
        search_label = customtkinter.CTkLabel(
            self, text="Search for a Pokémon to get information!", text_color="white")
